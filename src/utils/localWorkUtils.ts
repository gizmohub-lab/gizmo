 ...
 * - Not Paid: Amount Got = 0
 * - Partially Paid: Amount Got > 0 and < Total Amount
 * - Paid: Amount Got = Total Amount
 * - Overpaid: Amount Got > Total Amount
 */
export function calculatePaymentStatus(totalAmount: number, amountGot: number): PaymentStatus {
  const total = Math.max(0, Number(totalAmount) || 0);
  const got = Math.max(0, Number(amountGot) || 0);
  if (got === 0) return 'Not Paid';
  if (got < total) return 'Partially Paid';
  if (got === total) return 'Paid';
  return 'Overpaid';
}

/**
 * Automatically calculates Amount To Get:
 * Total Amount - Amount Got
 * Does not allow negative Amount To Get.
 */
export function calculateAmountToGet(totalAmount: number, amountGot: number): number {
  const total = Math.max(0, Number(totalAmount) || 0);
  const got = Math.max(0, Number(amountGot) || 0);
  return Math.max(0, total - got);
}
...
export function getWorkFinancials(work: Partial<LocalWork>): {
  ...
  let amountGot = Number(work.amountGot);
  if (isNaN(amountGot) || work.amountGot === undefined) {
  ...
  amountGot = Math.max(0, amountGot);
